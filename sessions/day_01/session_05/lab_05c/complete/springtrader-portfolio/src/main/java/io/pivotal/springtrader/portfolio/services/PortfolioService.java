...
public class PortfolioService {
	...
	public Order addOrder(Order order) {
		...
		if (order.getOrderType() == OrderType.BUY) {
			...
			ResponseEntity<Double> result= restTemplate.getForEntity(accountsUrl + "/accounts/{userid}/decreaseBalance/{amount}", Double.class, order.getAccountId(), amount);
			return saveOrder(order, result);
		} else {
			double amount = order.getQuantity()*order.getPrice().doubleValue()-order.getOrderFee().doubleValue();
			ResponseEntity<Double> result= restTemplate.getForEntity(accountsUrl + "/accounts/{userid}/increaseBalance/{amount}", Double.class, order.getAccountId(), amount);
			return saveOrder(order,result);
		}
